...
// 
$.post('/getMyData', {
    id: sessionStorage.getItem('user_M')
}, (response) => {
    response = JSON.parse(response);
    // 
    console.log(response);
    // 
    document.getElementById('nom').value = response.NOM_PAT;
    document.getElementById('prenom').value = response.Prenom_PAT;
    document.getElementById('matricule').value = response.MATRICULE_PAT;
    document.getElementById('dateN').value = response.Date_Naissence;
    document.getElementById('numero').value = response.Tel;
});